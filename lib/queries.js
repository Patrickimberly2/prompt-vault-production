...
const MOCK_PROMPTS = [
  {
    ...
    prompt_text: "Create an engaging blog post about [TOPIC]. Include an attention-grabbing headline, introduction, main points, and conclusion.",
  ...
  },
  {
    id: 2,
    name: "Code Reviewer",
    prompt_text: "Review the following code for best practices, potential bugs, and improvements: [CODE]",
  ...
  },
  {
    ...
    prompt_text: "Write a professional email for [SITUATION]. Make it clear, concise, and appropriate for the context.",
    category: "Business",
...
const MOCK_CATEGORIES = ["All", "Content Creation", "Development", "Business", "Marketing", "Education"];

// Check if we're in development mode
const isDevelopmentMode = !supabase;
...
export async function searchPrompts(searchTerm = '', category = 'all', tags = [], page = 1, limit = 50) {
  ...
  if (isDevelopmentMode) {
    ...
    return {
      data: filteredPrompts.slice((page - 1) * limit, page * limit),
...
/**
 * Legacy search function to maintain compatibility with existing page.js
 * @param {Object} options
 * @returns {Promise<{data: Prompt[], count: number, error: Error|null}>}
 */
export async function legacySearchPrompts({
  searchTerm = '',
  category = '',
  tags = [],
  aiModel = '',
  ...
  try {
    const result = await searchPrompts(searchTerm, category === '' ? 'all' : category, tags, page, limit);
    return { ...result, error: null };
  } catch (error) {
    console.error('Error in legacy search prompts:', error);
...
export async function getCategories() {
  ...
  if (isDevelopmentMode) {
    return { 
      data: MOCK_CATEGORIES.map((name, index) => ({ 
        id: index, 
        name, 
        sort_order: index 
      })), 
